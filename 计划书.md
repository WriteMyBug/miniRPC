...
- [x] 单机 echo RPC QPS >= 3 万（Release：40 并发实测 31587 QPS）
- [x] P99 延迟 <= 5 ms（60 并发内 P99 <= 4.4ms）
- [x] 压测报告含 CPU、内存、连接数变化（benchmark/report.md）
...
│   └── ...            # 与 include 对应
├── proto/             # .proto 定义
├── registry/          # 注册中心（RegistryServiceImpl + registry_server）
├── examples/          # echo / calculator / 并发示例
├── benchmark/         # 压测程序与报告（report.md）
├── docs/              # decisions.md / interview-qa.md
└── tests/             # 单元与集成测试
```
...
- [x] 查看 `/home/nina/cpp_learn/threadpool` 现有实现，决定复用或改造（复用 + 移植，见 docs/decisions.md）
- [x] 线程池（任务队列 + 条件变量，含优先级/回调模式）
- [x] 协议头定义（魔数/版本/类型/序列号/长度，固定 24 字节）
- [x] protoc 接入 CMake，定义 Echo/Calculator proto
- [x] Codec 编解码与半包测试
- 验收：tests/codec_test 覆盖拆包边界（逐字节/任意切分/粘包/非法报文，2026-08-02 通过）；echo 服务接入线程池，16 客户端 × 50 消息共 800 请求 0 失败

### 第 3 周：RPC 核心

目标：一次完整 RPC 调用。

- [x] Service 抽象与注册表（protobuf Service + 完整名注册表）
- [x] RpcServer 接收请求并分发（Codec 拆包 + 线程池 + 注册表查分）
- [x] RpcChannel 客户端封装（同步调用，序列号匹配响应）
- [x] RpcController：错误码、超时
- [x] 超时重试（每次重试新序列号，服务端业务错误不重试）
- 验收：examples/calculator 从客户端调用成功（2026-08-02 实测）；README 已说明调用流程

### 第 4 周：注册中心、负载均衡与压测

目标：完整分布式形态 + 数据说话。

- [x] 注册中心：服务注册/拉取/心跳剔除（注册中心本身是一个 RPC 服务，见 docs/decisions.md D5）
- [x] 轮询 + 一致性哈希负载均衡（虚拟节点，loadbalancer_test 覆盖分布与最小迁移）
- [ ] 异步客户端（可选加分项，暂缓；同步通道串行化已够演示，见 D6）
- [x] 压测程序：QPS、P99、CPU/内存（benchmark/report.md）
- [x] 架构图、README、压测报告、面试 Q&A（docs/interview-qa.md）
- 验收：双服务节点 + 注册中心演示故障剔除（registry_test 通过，实测 node-b 心跳超时被剔除）；压测报告产出（40 并发 31587 QPS、P99 2.6ms）
