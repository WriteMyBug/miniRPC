...
| protobuf | 未安装（`protoc` 与 libprotobuf-dev 均无） | 第 1 周待安装 |
| WSLg / 挂载盘 | 可用（`/mnt/c`、`/mnt/wslg` 存在） | 日常开发不依赖，仅按需访问 |

**当前环境待办**：

- [ ] 安装 protobuf：`sudo apt update && sudo apt install -y protobuf-compiler libprotobuf-dev`
- [ ] （可选）安装 valgrind：`sudo apt install -y valgrind`
- [x] 首次提交（2026-08-02 完成，约定式 commit）

> 注：protobuf/valgrind 安装需要 sudo 密码，当前非交互环境无法代为执行，待用户在终端手动执行上述命令。
...
- [ ] 安装 protobuf（protoc + libprotobuf-dev，当前未安装）
- [x] CMake 工程 + 目录骨架
- [x] 日志模块（线程安全、分级输出）
